  ...
  bool blMakeAmbChSet)
{
  ...
  if (pAmbigGroups != NULL && pAmbigGroups->size() >= 2) {
    std::vector<w32string>::iterator g_it;
    for (g_it = pAmbigGroups->begin(); g_it != pAmbigGroups->end(); g_it++)
    {
      int nMatches = 0;
      w32string::iterator it;
      for (it = g_it->begin(); it != g_it->end(); it++) {
        if (chset.count(*it))
          nMatches++;
      }
  ...
  if (!sFileName.IsEmpty()) {
    ...
    try {
      std::unique_ptr<TStringFileStreamW> pFile(new TStringFileStreamW(
          sFileName, fmOpenRead, ceAnsi, true,
          65536, "\n\t "));
  ...
  double* pdEntropy)
{
  ...
  //try {
  std::unique_ptr<TStringFileStreamW> pSrcFile(new TStringFileStreamW(sSrcFileName,
      fmOpenRead, ceAnsi,
      true, 65536, "\n\t "));
...
int PasswordGenerator::LoadTrigramFile(WString sFileName)
{
  ...
  if (!sFileName.IsEmpty()) {
    ...
    try {
      ...
      pFile.reset(); // delete object and set ptr to NULL
